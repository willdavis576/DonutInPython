...
for i in range(0, 60):
    for j in range(0,60):
        x,y,z = genPoints(1,2, radians(6*i), radians(6*j), 1,1) #get torus coords, 0 -> 2pi. using 120 and scaling to have less processing power
        xList = np.append(xList, x)
        yList = np.append(yList, y)
        zList = np.append(zList, z)


data = [go.Scatter3d(x=xList, y=yList, z=zList,mode='markers', marker=dict(size=1))]
